    ...
    protCalPerc(){
        ...
        if((empiricalNonProtCal+empiricalProtCal)==0){
            return 0
        } 
        else{
            return empiricalProtCal/(empiricalNonProtCal+empiricalProtCal)*100;
        }
    };
    }
...
function calculate(){
    ...
    paragraphs[3].innerHTML='Общая энергетическая ценность: '+Math.round(result.energy)+' ккал (из них белковая: около '+Math.round(result.protCalPerc())+'%), ';
